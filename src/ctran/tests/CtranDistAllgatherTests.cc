...
TEST_F(CtranAllgatherTest, InplaceAllgatherRing) {
  ncclComm_t comm =
      createNcclComm(this->globalRank, this->numRanks, this->localRank);
  ...
  void* inplaceSendBuf = (char*)recvbuf + this->globalRank * sendBytes;
  auto res =
      ctranAllGatherRing(inplaceSendBuf, recvbuf, count, dt, comm, stream);
  EXPECT_EQ(res, ncclSuccess);
...
TEST_F(CtranAllgatherTest, OutOfPlaceAllgatherRing) {
  ncclComm_t comm =
      createNcclComm(this->globalRank, this->numRanks, this->localRank);
  ...
  auto res = ctranAllGatherRing(sendbuf, recvbuf, count, dt, comm, stream);
  EXPECT_EQ(res, ncclSuccess);
