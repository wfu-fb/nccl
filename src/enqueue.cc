...
ncclResult_t ncclEnqueueCheck(struct ncclInfo* info) {
  ...
  INFO(NCCL_COLL,"%s: opCount %lx sendbuff %p recvbuff %p count %zi datatype %d op %d root %d comm %p commHash %lu [nranks=%d, localRanks=%d] stream %p",
        info->opName, info->comm->opCount, info->sendbuff, info->recvbuff, info->count,
        info->datatype, info->op, info->root, info->comm, info->comm->commHash, info->comm->nRanks, info->comm->localRanks, info->stream);
